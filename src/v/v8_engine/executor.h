...
struct task final : work_item {
    ...
    // Run this function after wathdog was alarmed
    void on_timeout() noexcept override { _func.on_timeout(); }

    std::chrono::milliseconds get_timeout() const  noexcept override { return _timeout; }
...
public:
    ...
    // Stop executor. Stop queue with task and close gate
    ss::future<> stop();
    ...
    CONCEPT(
    template<typename T>
    concept FuncFoExecutorConcept = requires(T func) {
        { func() } -> void;
        { func.cancel() } -> void;
    ...
    CONCEPT(requires FuncFoExecutorConcept<WrapperFuncForExecutor>)
    ss::future<> submit(
      ...
      std::chrono::milliseconds timeout) {
        ...
        co_await std::move(new_task->get_future());
    }
