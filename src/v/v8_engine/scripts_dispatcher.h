...
public:
    ...
    ss::future<> update_script(
      const model::topic_namespace& topic, model::data_policy& data_policy) {
        co_await delete_script(topic);

        auto code = get_code(data_policy);
        _scripts.emplace(
          topic,
          ss::make_lw_shared<script_guard<Executor>>(
            data_policy, std::move(code)));
        co_return;
    }

    ss::future<> delete_script(const model::topic_namespace& topic) {
        auto it = _scripts.find(topic);
        if (it == _scripts.end()) {
            co_return;
        }

        ss::weak_ptr<script_guard<Executor>> weak_script_ptr(
          it->second->weak_from_this());
        _scripts.erase(it);

        co_await ss::do_until(
          [weak_script_ptr = std::move(weak_script_ptr)] {
              return weak_script_ptr.get() == nullptr;
          },
          [] { return ss::now(); });
    }
