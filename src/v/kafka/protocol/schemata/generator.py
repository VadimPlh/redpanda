...
from flex_versions import flex_versions
...
{%- else %}
    ...
    void decode_pre_flex(iobuf, api_version);
{%- endif %}
{%- endif %}
};
...
{%- if first_flex < 0 %}
{{- caller() }}
{%- elif flex and field.versions().min <= first_flex -%}
{{- caller() }}
{%- elif not flex and field.versions().min >= first_flex %}
{%- else %}
...
void {{ struct.name }}::encode_pre_flex([[maybe_unused]] response_writer& writer, [[maybe_unused]] api_version version) {
...
void {{ struct.name }}::decode(request_reader& reader, api_version version) {
    if (version >= api_version({{ first_flex }})) {
        ...
        decode_pre_flex(reader, version);
    }
}
...
void {{ struct.name }}::decode_pre_flex([[maybe_unused]] request_reader& reader, [[maybe_unused]] api_version version) {
...
void {{ struct.name }}::decode(iobuf buf, api_version version) {
    if (version >= api_version({{ first_flex }})) {
        ...
        decode_pre_flex(std::move(buf), version);
    }
}
...
void {{ struct.name }}::decode_pre_flex(iobuf buf, [[maybe_unused]] api_version version) {
    request_reader reader(std::move(buf));
...
if __name__ == "__main__":
    ...
    # version branching, since all versions since 0 were always flexible
    #
    # A first_flex value of -1 indicates the value of flexibleVersions was set to 'none'.
    # These requests will always be interpreted as non-flex and produce a conditional to
    ...
    first_flex = flex_versions[op_req]
