...
using copyable_tags = std::vector<std::tuple<uint32_t, std::optional<bytes>>>;

copyable_tags make_random_tags(size_t n) {
    copyable_tags tags;
    for (uint32_t i = 0; i < n; ++i) {
        const auto is_null = (random_generators::get_int(10) % 2) == 0;
        tags.emplace_back(
          n,
          (is_null ? std::optional<bytes>() : random_generators::get_bytes()));
    }
    return tags;
}

kafka::tagged_fields convert_copyable_tags(copyable_tags ctags) {
    kafka::tagged_fields tags;
    std::transform(
      ctags.begin(), ctags.end(), std::back_inserter(tags), [](auto& t) {
          auto& [tag, item] = t;
          return std::make_tuple(
            tag,
            item ? bytes_to_iobuf(std::move(*item)) : std::optional<iobuf>());
      });
    return tags;
}
...
SEASTAR_THREAD_TEST_CASE(serde_tags) {
    ...
    writer.write_tags(std::move(convert_copyable_tags(tags)));
    ...
    BOOST_REQUIRE(convert_copyable_tags(tags) == deser_tags);
...
SEASTAR_THREAD_TEST_CASE(serde_flex_types) {
    {
        /// flex strings
        auto str = random_generators::gen_alphanum_string(15);
        BOOST_CHECK_EQUAL(str, serde_flex(str));
    }
    {
        /// optional flex strings
        auto opt_str = std::optional<ss::sstring>(
    ...
    {
        /// flex iobuf
        auto str = random_generators::gen_alphanum_string(10);
