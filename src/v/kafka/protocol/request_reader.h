...
public:
    ...
    tagged_fields read_tags() {
        tagged_fields tags;
        auto num_tags = read_unsigned_varint();
        while (num_tags-- > 0) {
            auto tag_id = read_unsigned_varint();
            auto tag = read_fragmented_nullable_flex_bytes();
            tags.emplace_back(tag_id, std::move(tag));
        }
        return tags;
    }
    ...
    void consume_tags() {
        ...
        auto num_tags = read_unsigned_varint();
        while (num_tags-- > 0) {
            (void)read_unsigned_varint(); // consume tag
            auto next_len = read_unsigned_varint();
            if (next_len > 0) {
                // consume whatever type the tag value is
                _parser.skip(next_len - 1);
            }
        }
    }
